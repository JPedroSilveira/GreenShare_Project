...
public class UserAchievementServiceImpl extends IsHelper implements UserAchievementService {
	...
	public ResponseEntity<?> save(UserAchievement userAchievement) {
		if (isNotNull(userAchievement)) {
			...
			Iterable<UserAchievement> userAchievementListDB = userAchievementRepository.findAllByUserAndAchievement(userDB, userAchievement.getAchievement()); 
			if(isNull(userAchievementListDB)) {
				...
				return newUserAchievement.isValid() ? new ResponseEntity<UserAchievement>(userAchievementRepository.save(newUserAchievement), HttpStatus.OK)
						: new ResponseEntity<List<String>>(newUserAchievement.getValidationErrors(), HttpStatus.BAD_REQUEST);
			}
	...
	public ResponseEntity<?> delete(Long id) {
		if (isNotNull(id)) {
			...
			return new ResponseEntity<String>("Relação conquista usuário não pertence ao usuário logado.", HttpStatus.UNAUTHORIZED);
		}
